...
static short wave_epollevent_2poll(uint32_t events)
{
	short e = 0;
	if (events & EPOLLIN)
		e |= POLLIN;
...
static int wave_poll_inner(struct pollfd *fds, nfds_t nfds, int timeout)
{
	...
	wave_coroutine *co = sched->curr_thread;

	for (int i = 0; i < nfds; i++)
	{

		struct epoll_event ev;
		ev.events = wave_pollevent_2epoll(fds[i].events);
		ev.data.fd = fds[i].fd;
		epoll_ctl(sched->poller_fd, EPOLL_CTL_ADD, fds[i].fd, &ev);

		co->events = fds[i].events;
		wave_schedule_sched_wait(co, fds[i].fd, fds[i].events, timeout);
	}
	...
	for (int i = 0; i < nfds; i++)
	{
		...
		wave_schedule_desched_wait(fds[i].fd);
	}
...
ssize_t wave_sendto(int fd, const void *buf, size_t len, int flags,
...
ssize_t wave_recvfrom(int fd, void *buf, size_t len, int flags,
